...
public class MazeProblem {
    ...
    public final List<MazeState> GOAL_STATES;
    ...
    MazeProblem (String[] maze) {
        ...
        KEY_STATE = foundKey;
        GOAL_STATES = Collections.unmodifiableList(foundGoals);
    }
    ...
    /**
     * Given a MazeState, returns the cost associated with moving into that state in the MazeProblem's maze.
     * 
     * @param state A MazeState (col, row) to enter
     * @return Integer representing the cost of moving into the given state.
     */
    public int getCost (MazeState state) {
    	return (this.maze[state.row].charAt(state.col) == 'M') ? 3 : 1; 
    }
